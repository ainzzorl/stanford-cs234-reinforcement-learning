...
class Linear(DQN):
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        value = torch.flatten(state, start_dim = 1)
        out = network(value)
    ...
    def update_target(self):
        ...
        torch.save(self.q_network.state_dict(), '/tmp/q')
        self.target_network.load_state_dict(torch.load('/tmp/q'))
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        # q_samp = rewards + gamma * torch.max(target_q_values, 1)[0] * (torch.nn.functional.one_hot(done_mask.to(torch.int64))[:, 0])
        # q_a = torch.sum(q_values * torch.nn.functional.one_hot(actions.to(torch.int64), num_classes=num_actions), dim=-1)
        # return torch.nn.functional.mse_loss(q_samp, q_a)
